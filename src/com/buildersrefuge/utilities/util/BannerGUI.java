...
public class BannerGUI {
	...
	public Inventory generateStartInv(){
		...
		Inventory inv = Bukkit.createInventory(null, 54, "§1Select a base color");
		for (int x = 0; x<54; x++){
			inv.setItem(x, i.create(Material.STAINED_GLASS_PANE, (short) 7, 1, "&7", ""));
		}
	...
	public Inventory generateColorInv(Inventory inv, ItemStack clicked, boolean first){
		Items i = new Items();
		BannerUtil bU = new BannerUtil();
		Inventory inv1 = Bukkit.createInventory(null, 54, "§1Select a color");
		for (int x = 0; x<54; x++){
		...
		if (first){
			inv1.setItem(4, bU.createBanner("&aClick to get the banner", 1, bU.getBaseColor(clicked), ""));
		}
		else {
		...
		for (int x = 19; x<27; x++){
			inv1.setItem(x, i.create(Material.INK_SACK, (short)(x-19), 1, "&3" + bU.getAllColors().get(x-19).toString().toLowerCase().replace("_", " "), "&7__&7click to select"));
			}
		for (int x = 28; x<36; x++){
			inv1.setItem(x, i.create(Material.INK_SACK, (short)(x-20), 1, "&3" + bU.getAllColors().get(x-20).toString().toLowerCase().replace("_", " "), "&7__&7click to select"));
		}
		return inv1;
	}
	...
	public Inventory generatePatternInv(Inventory inv, ItemStack clicked){
		...
		if (click.equals(DyeColor.WHITE)||click.equals(DyeColor.SILVER)||click.equals(DyeColor.LIME)||click.equals(DyeColor.LIGHT_BLUE)||click.equals(DyeColor.YELLOW)){base=DyeColor.BLACK;}
		for (int x = 9; x<47; x++){
